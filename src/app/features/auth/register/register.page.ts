...
@Component({
  ...
  template: `
    <div class="flex flex-col gap-6 animate-fade-in">
      ...
      <form (ngSubmit)="onSubmit()" class="flex flex-col gap-4">
        ...
        <div>
          ...
          <div class="relative">
            ...
            >
              @if (showPassword()) {
                <svg lucideEyeOff class="w-5 h-5" aria-hidden="true"></svg>
              } @else {
                <svg lucideEye class="w-5 h-5" aria-hidden="true"></svg>
              }
            </button>
          </div>
        </div>
        ...
        >
          @if (isLoading()) {
            ...
            <svg lucideUserPlus class="w-[18px] h-[18px]" strokeWidth="2" aria-hidden="true"></svg>
          }
      ...
      <button (click)="onGoogleSignIn()" class="w-full py-3 rounded-xl border border-white/10 text-on-surface font-medium hover:bg-surface-hover transition-colors flex items-center justify-center gap-2">
